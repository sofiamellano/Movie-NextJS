                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                    {seriesProvider.length > 0 ? (
                        seriesProvider.map((provider) => (
                            <div key={provider.provider_id} className="bg-gray-800 rounded-lg overflow-hidden shadow-lg hover:scale-105 transform transition duration-300 ease-in-out h-64">
                                {provider.logo_path ? (
                                    <img
                                ...
                                <div className="p-4">
                                    <h3 className="text-white text-xl font-semibold text-center">{provider.provider_name || 'Unknown Provider'}</h3>
                                </div>
                            </div>
                        ))
                    ) : null}
